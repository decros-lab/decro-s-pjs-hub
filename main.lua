...
local scytheLowCD,swampLowCD,willLowCD,warFanLowCD,iceLowCD,waterLowCD,bloodLowCD,reaperLowCD,shockwaveLowCD,dreamLowCD,blockLowCD,tamariLowCD,arrowLowCD,soundLowCD,windLowCD,mistLowCD,thunderLowCD,insectLowCD,snowLowCD,beastLowCD,flameLowCD,breathLowCD=false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false
...
-- [2026] Global Maid/Janitor logic for background tasks
local globalConnections={}
local charConnections={}
local floodConnections={}

local function cleanupConnections(list)
    for _,c in ipairs(list) do if c.Connected then c:Disconnect() end end
    table.clear(list)
end

local function setupSpeedHook(character)
    cleanupConnections(charConnections)
...
table.insert(globalConnections, LocalPlayer.CharacterAdded:Connect(function(char) currentTrueSpeed=16.80 setupSpeedHook(char) end))

-- [2026] Event-based optimization for No_Sun_Damage (Instead of while true)
local function applySunDamageImmunity(char)
    if not noSunDamageEnabled then return end
    ...
    if myValues and not myValues:FindFirstChild("No_Sun_Damage") then
        ...
        b.Name="No_Sun_Damage"
        b.Value=true
        b.Parent=myValues
    end
end
table.insert(globalConnections, LocalPlayer.CharacterAdded:Connect(function(char) task.wait(1) applySunDamageImmunity(char) end))

-- [2026] Single Heartbeat for Stamin/Breathing
table.insert(globalConnections, RunService.Heartbeat:Connect(function()
    ...
    if infinityStaminaEnabled or infinityBreathingEnabled then
        pcall(function()
            ...
            if not myValues then return end
...
-- [2026] Optimal Cooldown Cache System
local cachedCooldownValues={}
local function handleNewCooldown(desc)
    if desc.Name=="CoolDown" and desc:IsA("ValueBase") then
        ...
        table.insert(cachedCooldownValues,desc)
    end
end
local function initCooldownCache()
    table.clear(cachedCooldownValues)
    local powerAdder=LocalPlayer.PlayerGui:FindFirstChild("Power_Adder")
    if powerAdder then
        for _,desc in ipairs(powerAdder:GetDescendants()) do handleNewCooldown(desc) end
        table.insert(globalConnections, powerAdder.DescendantAdded:Connect(handleNewCooldown))
        table.insert(globalConnections, powerAdder.DescendantRemoving:Connect(function(desc)
            if desc.Name=="CoolDown" then
                local idx=table.find(cachedCooldownValues,desc)
                if idx then table.remove(cachedCooldownValues,idx) end
                originalCooldowns[desc]=nil
            end
        end))
    end
end
task.spawn(initCooldownCache)
table.insert(globalConnections, LocalPlayer.PlayerGui.DescendantAdded:Connect(function(desc) if desc.Name=="Power_Adder" then task.wait(0.5) initCooldownCache() end end))

task.spawn(function()
    while not scriptUnloaded do
        for _,desc in ipairs(cachedCooldownValues) do
            pcall(function()
                ...
                if shouldReduce then
                    local factor=(dreamSkills[skillName] and dreamLowCD) and 0.9 or 0.3
                    local targetVal=originalCooldowns[desc]*factor
                    if desc.Value~=targetVal then desc.Value=targetVal end
                else
                    if originalCooldowns[desc] and desc.Value~=originalCooldowns[desc] then desc.Value=originalCooldowns[desc] end
                end
            end)
        end
        task.wait(0.3)
...
for _,name in ipairs({"Lantern_Handler","Box_Lantern_Handler","Old_Lantern_Handler","Lantern_Of_Despair_Handler","Lantern_Of_Everlasting_Glow_Handler"}) do pcall(function() local r=ReplicatedStorage.Remotes:FindFirstChild(name) if r then table.insert(lanternHandlers,r) end end) end
local lanternTools={}
for _,name in ipairs({"Lantern","Box Lantern","Old Lantern","Lantern Of Despair","Lantern Of Everlasting Glow"}) do pcall(function() local t=ReplicatedStorage.Tools:FindFirstChild(name) if t then table.insert(lanternTools,t) end end) end
local toolsData=nil
...
local function fireCrash()
    ...
    isActive=true
    if invokeRemote then
        for i=1,INVOKE_THREADS do
            task.spawn(function()
                while isActive and not scriptUnloaded do
    ...
    table.insert(floodConnections, RunService.Heartbeat:Connect(function()
        for _,handler in ipairs(lanternHandlers) do
            for _,tool in ipairs(lanternTools) do
                pcall(function() handler:FireServer(2,tool) end)
                pcall(function() handler:FireServer(1,tool) end)
            end
        end
        if toolsData then
            for _,v in ipairs(toolsData:GetChildren()) do
...
local function PlayTween(instance,goal)
    ...
    tween:Play()
    tween.Completed:Connect(function() if Library.Tweens[instance]==tween then Library.Tweens[instance]=nil end end)
    return tween 
end
...
local function MakeDraggable(topbarobject,object)
    ...
    table.insert(Library.Connections,topbarobject.InputBegan:Connect(function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then Dragging=true DragStart=input.Position StartPosition=object.Position end 
    end))
    table.insert(Library.Connections,UserInputService.InputChanged:Connect(function(input)
        ...
        if Dragging and input==DragInput then
            ...
            object.Position=UDim2.new(StartPosition.X.Scale,math.floor(StartPosition.X.Offset+Delta.X),StartPosition.Y.Scale,math.floor(StartPosition.Y.Offset+Delta.Y)) 
        end 
    end))
    table.insert(Library.Connections,UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
            if Dragging then
                ...
                if (objCenter-(screenSize/2)).Magnitude<30 then PlayTween(object,{Position=UDim2.new(0.5,0,0.5,0)}) end 
            end 
        end 
    end)) 
end
...
function Library:Unload()
    cleanupConnections(self.Connections)
    ...
    cleanupConnections(charConnections)
    cleanupConnections(globalConnections)
    cleanupConnections(floodConnections)
    self.Connections={}
    self.Tweens={}
    self.Instances={}
    scriptUnloaded=true 
...
function Library:Window(options)
    ...
    Make("TextLabel",{Text=Name.." <font color='rgb(150,150,150)'>"..SubName.."</font>",Size=UDim2.new(1,-40,1,0),Position=UDim2.new(0,10,0,0),BackgroundTransparency=1,TextColor3=self.Settings.Theme.Text,TextXAlignment=Enum.TextXAlignment.Left,Font=Enum.Font.GothamBold,TextSize=14,RichText=true},Topbar)
    ...
    Make("UIListLayout",{Padding=UDim.new(0,2),HorizontalAlignment=Enum.HorizontalAlignment.Center,SortOrder=Enum.SortOrder.LayoutOrder},Sidebar)
    local WindowObj={CurrentTab=nil,Tabs={},Main=Main,MenuKeybind=options.MenuKeybind or Enum.KeyCode.J}
    table.insert(self.Connections,UserInputService.InputBegan:Connect(function(input,gp) if not gp and input.KeyCode==WindowObj.MenuKeybind then Main.Visible=not Main.Visible end end))
    function WindowObj:Page(pageOptions)
        ...
        table.insert(Library.Connections,PageLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() PageContainer.CanvasSize=UDim2.new(0,0,0,PageLayout.AbsoluteContentSize.Y+10) end))
        table.insert(Library.Connections,TabButton.MouseButton1Click:Connect(function()
            for _,t in pairs(WindowObj.Tabs) do t.Container.Visible=false PlayTween(t.Button,{TextColor3=Library.Settings.Theme.TextDark}) end
            PageContainer.Visible=true PlayTween(TabButton,{TextColor3=Library.Settings.Theme.Accent}) 
        end))
        local PageObj={Container=PageContainer,Button=TabButton}
        table.insert(WindowObj.Tabs,PageObj)
        if #WindowObj.Tabs==1 then PageContainer.Visible=true TabButton.TextColor3=Library.Settings.Theme.Accent end
        function PageObj:Section(secOptions)
            ...
            Make("UICorner",{CornerRadius=UDim.new(0,6)},SectionFrame) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},SectionFrame)
            Make("TextLabel",{Text="  "..SecName,Size=UDim2.new(1,0,0,25),BackgroundTransparency=1,TextColor3=Library.Settings.Theme.Text,TextXAlignment=Enum.TextXAlignment.Left,Font=Enum.Font.GothamBold,TextSize=13},SectionFrame)
            ...
            Make("UIListLayout",{Padding=UDim.new(0,4),SortOrder=Enum.SortOrder.LayoutOrder},InnerContainer) Make("UIPadding",{PaddingBottom=UDim.new(0,6),PaddingLeft=UDim.new(0,6),PaddingRight=UDim.new(0,6)},InnerContainer)
            local SectionObj={}
            function SectionObj:Button(btnOptions)
                local bName=btnOptions.Name or "Button" local bCall=btnOptions.Callback or function() end
                ...
                Make("UICorner",{CornerRadius=UDim.new(0,5)},Btn) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},Btn)
                ...
                table.insert(Library.Connections,Btn.MouseEnter:Connect(function() PlayTween(Btn,{BackgroundColor3=hC}) end)) table.insert(Library.Connections,Btn.MouseLeave:Connect(function() PlayTween(Btn,{BackgroundColor3=Library.Settings.Theme.Background,TextSize=12}) end))
                table.insert(Library.Connections,Btn.MouseButton1Down:Connect(function() PlayTween(Btn,{TextSize=11}) end)) table.insert(Library.Connections,Btn.MouseButton1Up:Connect(function() PlayTween(Btn,{TextSize=12}) end))
                table.insert(Library.Connections,Btn.MouseButton1Click:Connect(function() bCall() Btn.BackgroundColor3=Library.Settings.Theme.Outline PlayTween(Btn,{BackgroundColor3=hC}) end)) 
            end
            function SectionObj:Toggle(tOptions)
                local tName=tOptions.Name or "Toggle" local isLowCD=(tName:lower():find("low cd") or tName:lower():find("low cooldown")) and not tName:lower():find("all") local tCall=tOptions.Callback or function() end
                if isLowCD then local origCall=tCall tCall=function(Value) origCall(Value) if not updatingToggles and allLowCDToggle then updatingToggles=true local allEnabled=true for _,toggle in ipairs(cdToggles) do if not toggle:Get() then allEnabled=false break end end allLowCDToggle:Set(allEnabled) updatingToggles=false end end end
                ...
                Make("UICorner",{CornerRadius=UDim.new(0,4)},Box) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},Box)
                ...
                local function Fire() State=not State PlayTween(Box,{BackgroundColor3=State and Library.Settings.Theme.Accent or Library.Settings.Theme.Background}) PlayTween(Lbl,{TextColor3=State and Library.Settings.Theme.Text or Library.Settings.Theme.TextDark}) tCall(State) end
                ...
                function NestedObj:Set(val) if State==val then return end State=val PlayTween(Box,{BackgroundColor3=State and Library.Settings.Theme.Accent or Library.Settings.Theme.Background}) PlayTween(Lbl,{TextColor3=State and Library.Settings.Theme.Text or Library.Settings.Theme.TextDark}) tCall(State) end
                function NestedObj:Get() return State end
                if isLowCD then table.insert(cdToggles,NestedObj) end return NestedObj 
            end
            function SectionObj:Slider(sOptions)
                local sName=sOptions.Name or "Slider" local sMin=sOptions.Min or 0 local sMax=sOptions.Max or 100 local sDef=sOptions.Default or sMin local sCall=sOptions.Callback or function() end local parent=sOptions.ParentOverride or InnerContainer local Dragging=false local Value=sDef
                ...
                local BG=Make("TextButton",{Size=UDim2.new(1,0,0,10),Position=UDim2.new(0,0,0,20),BackgroundColor3=Library.Settings.Theme.Background,Text="",AutoButtonColor=false,BorderSizePixel=0},SldFrame) Make("UICorner",{CornerRadius=UDim.new(0,4)},BG) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},BG)
                local Fill=Make("Frame",{Size=UDim2.new((sDef-sMin)/(sMax-sMin),0,1,0),BackgroundColor3=Library.Settings.Theme.Accent,BorderSizePixel=0},BG) Make("UICorner",{CornerRadius=UDim.new(0,4)},Fill)
                local function UpdateLogic(input) local scale=math.clamp((input.Position.X-BG.AbsolutePosition.X)/BG.AbsoluteSize.X,0,1) Value=math.floor(sMin+(scale*(sMax-sMin))) ValLbl.Text=tostring(Value) PlayTween(Fill,{Size=UDim2.new(scale,0,1,0)}) sCall(Value) end
                table.insert(Library.Connections,BG.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then Dragging=true UpdateLogic(input) end end))
                table.insert(Library.Connections,UserInputService.InputEnded:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then Dragging=false end end))
                table.insert(Library.Connections,UserInputService.InputChanged:Connect(function(input) if Dragging and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then UpdateLogic(input) end end))
                local RetObj={} function RetObj:Set(v) Value=math.clamp(v,sMin,sMax) local scale=(Value-sMin)/(sMax-sMin) ValLbl.Text=tostring(Value) PlayTween(Fill,{Size=UDim2.new(scale,0,1,0)}) sCall(Value) end return RetObj 
            end
            function SectionObj:Dropdown(dOptions)
                local dName=dOptions.Name or "Dropdown" local dList=dOptions.Options or {} local dDef=dOptions.Default or "" local dCall=dOptions.Callback or function() end local parent=dOptions.ParentOverride or InnerContainer local Dropped=false
                local DropFrame=Make("Frame",{Size=UDim2.new(1,0,0,45),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,BorderSizePixel=0},parent) Make("TextLabel",{Text=dName,Size=UDim2.new(1,0,0,16),BackgroundTransparency=1,TextColor3=Library.Settings.Theme.Text,TextXAlignment=Enum.TextXAlignment.Left,Font=Enum.Font.Gotham,TextSize=12},DropFrame)
                local MainBtn=Make("TextButton",{Size=UDim2.new(1,0,0,25),Position=UDim2.new(0,0,0,20),BackgroundColor3=Library.Settings.Theme.Background,Text="  "..dDef,TextColor3=Library.Settings.Theme.TextDark,TextXAlignment=Enum.TextXAlignment.Left,Font=Enum.Font.Gotham,TextSize=12,AutoButtonColor=false,BorderSizePixel=0},DropFrame) Make("UICorner",{CornerRadius=UDim.new(0,5)},MainBtn) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},MainBtn)
                ...
                local Scroll=Make("ScrollingFrame",{Size=UDim2.new(1,0,0,0),Position=UDim2.new(0,0,0,50),BackgroundColor3=Library.Settings.Theme.Background,ScrollBarThickness=2,Visible=false,AutomaticSize=Enum.AutomaticSize.Y,BorderSizePixel=0},DropFrame) Make("UICorner",{CornerRadius=UDim.new(0,5)},Scroll) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},Scroll)
                local SLayout=Make("UIListLayout",{Padding=UDim.new(0,2)},Scroll) table.insert(Library.Connections,SLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() Scroll.CanvasSize=UDim2.new(0,0,0,SLayout.AbsoluteContentSize.Y) end))
                table.insert(Library.Connections,MainBtn.MouseButton1Click:Connect(function() Dropped=not Dropped Scroll.Visible=Dropped Icon.Text=Dropped and "-" or "+" if Dropped then local limit=math.clamp(SLayout.AbsoluteContentSize.Y,0,120) Scroll.Size=UDim2.new(1,0,0,limit) else Scroll.Size=UDim2.new(1,0,0,0) end end))
                local DropObj={} function DropObj:Refresh(newList) for _,v in ipairs(Scroll:GetChildren()) do if v:IsA("TextButton") then v:Destroy() end end for _,opt in ipairs(newList) do local btn=Make("TextButton",{Size=UDim2.new(1,-4,0,20),Position=UDim2.new(0,2,0,0),BackgroundTransparency=1,Text=opt,TextColor3=Library.Settings.Theme.TextDark,Font=Enum.Font.Gotham,TextSize=12,BorderSizePixel=0},Scroll) table.insert(Library.Connections,btn.MouseButton1Click:Connect(function() MainBtn.Text="  "..opt Dropped=false Scroll.Visible=false Icon.Text="+" dCall(opt) end)) end if Dropped then Scroll.Size=UDim2.new(1,0,0,math.clamp(SLayout.AbsoluteContentSize.Y,0,120)) end end
                function DropObj:Set(val) MainBtn.Text="  "..val dCall(val) end DropObj:Refresh(dList) return DropObj 
            end
            function SectionObj:Keybind(kOptions)
                local kName=kOptions.Name or "Keybind" local kDef=kOptions.Default or Enum.KeyCode.Unknown local kCall=kOptions.Callback or function() end local parent=kOptions.ParentOverride or InnerContainer local Binding=false local CurrentKey=kDef
                local KFrame=Make("Frame",{Size=UDim2.new(1,0,0,20),BackgroundTransparency=1,BorderSizePixel=0},parent) Make("TextLabel",{Text=kName,Size=UDim2.new(1,-60,1,0),BackgroundTransparency=1,TextColor3=Library.Settings.Theme.Text,TextXAlignment=Enum.TextXAlignment.Left,Font=Enum.Font.Gotham,TextSize=12},KFrame)
                local KBtn=Make("TextButton",{Size=UDim2.new(0,50,1,0),Position=UDim2.new(1,-50,0,0),BackgroundColor3=Library.Settings.Theme.Background,Text=CurrentKey.Name,TextColor3=Library.Settings.Theme.TextDark,Font=Enum.Font.Gotham,TextSize=11,AutoButtonColor=false,BorderSizePixel=0},KFrame) Make("UICorner",{CornerRadius=UDim.new(0,5)},KBtn) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},KBtn)
                table.insert(Library.Connections,KBtn.MouseButton1Click:Connect(function() Binding=true KBtn.Text="..." end)) table.insert(Library.Connections,UserInputService.InputBegan:Connect(function(input,gp) if Binding and input.UserInputType==Enum.UserInputType.Keyboard then Binding=false CurrentKey=input.KeyCode KBtn.Text=CurrentKey.Name kCall(CurrentKey) elseif not Binding and input.KeyCode==CurrentKey and not gp then kCall(CurrentKey) end end)) 
            end
            function SectionObj:Colorpicker(cOptions)
                local cName=cOptions.Name or "Colorpicker" local cDef=cOptions.Default or Color3.new(1,1,1) local cCall=cOptions.Callback or function() end local parent=cOptions.ParentOverride or InnerContainer
                local CFrame=Make("Frame",{Size=UDim2.new(1,0,0,20),BackgroundTransparency=1,BorderSizePixel=0},parent) Make("TextLabel",{Text=cName,Size=UDim2.new(1,-30,1,0),BackgroundTransparency=1,TextColor3=Library.Settings.Theme.Text,TextXAlignment=Enum.TextXAlignment.Left,Font=Enum.Font.Gotham,TextSize=12},CFrame)
                local Displayer=Make("TextButton",{Size=UDim2.new(0,24,0,14),Position=UDim2.new(1,-24,0.5,-7),BackgroundColor3=cDef,Text="",BorderSizePixel=0},CFrame) Make("UICorner",{CornerRadius=UDim.new(0,4)},Displayer) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},Displayer) table.insert(Library.Connections,Displayer.MouseButton1Click:Connect(function() cCall(cDef) end)) 
            end
            function SectionObj:Textbox(tOptions)
                local tName=tOptions.Name or "Textbox" local tPlaceholder=tOptions.Placeholder or "" local tCall=tOptions.Callback or function() end local parent=tOptions.ParentOverride or InnerContainer
                local TFrame=Make("Frame",{Size=UDim2.new(1,0,0,45),BackgroundTransparency=1,BorderSizePixel=0},parent) Make("TextLabel",{Text=tName,Size=UDim2.new(1,-10,0,20),Position=UDim2.new(0,5,0,0),BackgroundTransparency=1,TextColor3=Library.Settings.Theme.Text,TextXAlignment=Enum.TextXAlignment.Left,Font=Enum.Font.Gotham,TextSize=12},TFrame)
                local TBox=Make("TextBox",{Size=UDim2.new(1,-10,0,20),Position=UDim2.new(0,5,0,22),BackgroundColor3=Library.Settings.Theme.Background,PlaceholderText=tPlaceholder,Text="",TextColor3=Library.Settings.Theme.TextDark,PlaceholderColor3=Color3.fromRGB(120,120,120),Font=Enum.Font.GothamMedium,TextSize=12,ClearTextOnFocus=false,BorderSizePixel=0},TFrame) Make("UICorner",{CornerRadius=UDim.new(0,4)},TBox) Make("UIStroke",{Color=Library.Settings.Theme.Outline,Thickness=1.2},TBox) table.insert(Library.Connections,TBox.FocusLost:Connect(function() tCall(TBox.Text) end)) 
            end
            return SectionObj 
        end return PageObj 
    end return WindowObj 
end

function Library:Log(logOptions)
    if not self.LogsEnabled then return end local Title=logOptions.Title or "Log" local Text=logOptions.Text or "" local Duration=logOptions.Duration or 3
    ...
    if not LogContainer:FindFirstChild("UIListLayout") then Make("UIListLayout",{Padding=UDim.new(0,5),VerticalAlignment=Enum.VerticalAlignment.Bottom,SortOrder=Enum.SortOrder.LayoutOrder},LogContainer) end
    local Item=Make("Frame",{Size=UDim2.new(1,0,0,50),BackgroundColor3=self.Settings.Theme.Background,BackgroundTransparency=1,BorderSizePixel=0},LogContainer) Make("UICorner",{CornerRadius=UDim.new(0,6)},Item) local Stroke=Make("UIStroke",{Color=self.Settings.Theme.Outline,Thickness=1.2,Transparency=1},Item)
    ...
    PlayTween(Item,{BackgroundTransparency=0.1}) PlayTween(Stroke,{Transparency=0}) PlayTween(TLbl,{TextTransparency=0}) PlayTween(BLbl,{TextTransparency=0})
    task.delay(Duration,function() if scriptUnloaded then return end PlayTween(Item,{BackgroundTransparency=1}) PlayTween(Stroke,{Transparency=1}) PlayTween(TLbl,{TextTransparency=1}) local t=PlayTween(BLbl,{TextTransparency=1}) t.Completed:Connect(function() Item:Destroy() end) end) 
end

function Library:CreateSettingsPage(WindowObj)
    local SettingsPage=WindowObj:Page({Name="Settings"}) local SecUI=SettingsPage:Section({Name="UI Configuration"})
    SecUI:Keybind({Name="Menu Toggle Key",Default=WindowObj.MenuKeybind,Callback=function(key) WindowObj.MenuKeybind=key end})
    SecUI:Button({Name="Close panel",Callback=function() isActive=false Library:Unload() end}) 
end
...
SecServerDmg:Button({Name="Server kill",Callback=function()
    ...
    local args={[1]="ricespiritdamage",[2]=LocalPlayer.Character,[3]=CFrame.new(-362.226,425.48,-2354.54,0.32,0.02,0.94,0,1,-0.02,-0.94,0.008,0.32),[4]=99999999999999}
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("To_Server"):WaitForChild("Handle_Initiate_S"):FireServer(unpack(args))
    task.wait(1.5) TeleportService:Teleport(game.PlaceId) 
end})
local SecServerMisc=ServerTab:Section({Name="Misc"})
SecServerMisc:Button({Name="Rejoin",Callback=function() local placeId=game.PlaceId local jobId=game.JobId if placeId and jobId and #jobId>0 then TeleportService:TeleportToPlaceInstance(placeId,jobId) end end})
...
getgenv().selectedPlayerName="" getgenv().TargetPlayersArrow=false getgenv().AllArrow=false
SecServerKillAll:Textbox({Name="Player Target",Placeholder="zdecro, RobloxServerTest_1",Callback=function(v) getgenv().selectedPlayerName=v end})
SecServerKillAll:Toggle({Name="Arrow Aura [Target Player]",Default=false,Callback=function(v) getgenv().TargetPlayersArrow=v end})
SecServerKillAll:Toggle({Name="Arrow Aura [All Players]",Default=false,Callback=function(v) getgenv().AllArrow=v end})
...
SecLocalChar:Button({Name="Reset WalkSpeed",Callback=function()
    speedMultiplier=1 local character=LocalPlayer.Character local humanoid=character and character:FindFirstChildOfClass("Humanoid")
    if humanoid then isModifyingSpeed=true humanoid.WalkSpeed=currentTrueSpeed isModifyingSpeed=false end 
end})

-- [2026 FIX] Placed immediately after Reset WalkSpeed
...
SecLocalChar:Toggle({Name="No Sun Damage",Default=false,Callback=function(Value)
    ...
    if not noSunDamageEnabled then
        ...
        applySunDamageImmunity(LocalPlayer.Character)
    end 
end})
SecLocalChar:Toggle({Name="Infinity Stamina",Default=false,Callback=function(Value)
    local val=(Value==true) if infinityStaminaEnabled==val then return end infinityStaminaEnabled=val
    if val then origStamina=_G.Stamina origStamBreath=_G.StamBreath origRemoveStam=_G.RemoveStam origAddStam=_G.AddStamina _G.Stamina=function() return true end _G.StamBreath=function() return true end _G.RemoveStam=function() end _G.AddStamina=function() end
    else if origStamina then _G.Stamina=origStamina end if origStamBreath then _G.StamBreath=origStamBreath end if origRemoveStam then _G.RemoveStam=origRemoveStam end if origAddStam then _G.AddStamina=origAddStam end end 
end})
...
SecLocalChar:Toggle({Name="Infinity Breathing",Default=false,Callback=function(Value)
    local val=(Value==true) if infinityBreathingEnabled==val then return end infinityBreathingEnabled=val
...
allLowCDToggle=SecLocalCD:Toggle({Name="Enable all low cooldowns",Default=false,Callback=function(Value) updatingToggles=true local state=(Value==true) for _,tog in ipairs(cdToggles) do if tog~=breathLowCDToggle and tog~=blockLowCDToggle then tog:Set(state) end end updatingToggles=false end})
...
SecAnotherAB:Button({Name="Launch",Callback=function()
    local vim=game:GetService("VirtualInputManager") local player=Players.LocalPlayer local movesFolder=ReplicatedStorage:WaitForChild("Animations"):WaitForChild("Moves")
    ...
    for _,instance in ipairs(movesFolder:GetDescendants()) do cacheSkillAnimation(instance) end table.insert(globalConnections, movesFolder.DescendantAdded:Connect(cacheSkillAnimation))
    for _,instance in ipairs(clientModulesFolder:GetDescendants()) do cacheStartupModule(instance) end table.insert(globalConnections, clientModulesFolder.DescendantAdded:Connect(cacheStartupModule))
    ...
    table.insert(globalConnections, handleInitiateC.OnClientEvent:Connect(function(signalName,...) local name=string.lower(tostring(signalName or "")) if not startupModuleNames[name] then return end local sourceCharacter=nil for index=1,select("#",...) do sourceCharacter=extractCharacter(select(index,...)) if sourceCharacter then break end end tryBlockNearestCharacter(sourceCharacter,name) end))
    table.insert(globalConnections, RunService.Heartbeat:Connect(function() pcall(function() cleanupHandledTracks() cleanupHandledSignals() local character=player.Character local root=character and character:FindFirstChild("HumanoidRootPart") if not root then return end local nearestCharacter,nearestDistance=getNearestEnemyCharacter(root) if not nearestCharacter or nearestDistance>maxDistance then return end local nearestHumanoid=nearestCharacter:FindFirstChildOfClass("Humanoid") if not nearestHumanoid then return end for _,track in ipairs(nearestHumanoid:GetPlayingAnimationTracks()) do if isAbilityTrack(track) and not handledTracks[track] then handledTracks[track]=true press_F() break end end end) end)) 
end})

local function getClosestPlayerToTarget(selectedPlayerName)
    local closestPlayer=nil local closestDistance=math.huge local myRoot=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not myRoot then return nil end
    for _,p in ipairs(Players:GetPlayers()) do if p~=LocalPlayer and (not selectedPlayerName or p.Name:lower():find(selectedPlayerName:lower())) then local char=p.Character local root=char and char:FindFirstChild("HumanoidRootPart") if root then local dist=(root.Position-myRoot.Position).Magnitude if dist<closestDistance then closestDistance=dist closestPlayer=p end end end end return closestPlayer 
end

-- [2026] Background Tasks Refactored
task.spawn(function()
    while not scriptUnloaded do
...
task.spawn(function()
    while not scriptUnloaded do
        if getgenv().TargetPlayersArrow then
            local char=LocalPlayer.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                local selectedStr=getgenv().selectedPlayerName or "" local targets={}
                ...
                for _,targetName in ipairs(targets) do
                    if targetName~="" then
                        ...
                        if closestPlayer and closestPlayer.Character and closestPlayer.Character:FindFirstChild("HumanoidRootPart") then
                            for i=1,5 do if scriptUnloaded then break end pcall(function() game:GetService("ReplicatedStorage").Remotes.To_Server.Handle_Initiate_S:FireServer("arrow_knock_back_damage",char,closestPlayer.Character.HumanoidRootPart.CFrame,closestPlayer.Character,99999,99999) end) task.wait(0.2) end task.wait(1) 
                        end 
                    end 
                end 
            end 
    ...
    while not scriptUnloaded do
        if getgenv().AllArrow then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                if closestPlayer and closestPlayer.Character and closestPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    pcall(function() game:GetService("ReplicatedStorage").Remotes.To_Server.Handle_Initiate_S:FireServer("arrow_knock_back_damage",char,closestPlayer.Character.HumanoidRootPart.CFrame,closestPlayer.Character,99999,99999) end) task.wait(0.2)
                end 
            end 
        end task.wait(0.1) 
    end 
...
task.spawn(function()
    while not scriptUnloaded do
        if getgenv().InfWarFans then pcall(function() game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("war_Drums_remote"):FireServer(true) end) task.wait(2) else task.wait(1) end 
    end 
end)
...
task.spawn(function()
    ...
    while not scriptUnloaded do
        if antiDrownEnabled then
            ...
            if not cachedS then
                pcall(function() for _,t in pairs(getgc(true)) do if type(t)=="table" and rawget(t,"swim_bar") and type(rawget(t,"swim_bar"))=="table" and rawget(t,"max_swim_time") then weakSwimCache.t=t cachedS=t break end end end) 
            end
            ...
            if not cachedC then
                pcall(function()
                    for _,v in pairs(getgc(true)) do
                        if type(v)=="table" and rawget(v,"Increment") and rawget(v,"Decrement") and rawget(v,"Max") then weakClimbCache.t=v cachedC=v end
                        if not hasScannedClimbHooks and type(v)=="function" and not iscclosure(v) then
                            ...
                            if s and table.find(consts,"noclimb") and table.find(consts,"no climb") then
                                ...
                                origCheckInstance=hookfunction(v,function(p1) if infinityClimbEnabled then return true end return origCheckInstance(p1) end) 
                            end 
                        end 
                    end 
                end)
                ...
                for _,line in ipairs(res.Body:split("\n")) do
                    if line~="" then
                        ...
                        if s and d and d.message and not addedLogs[d.message] then
                            ...
                            SecLogs:Button({Name=d.message,Callback=function() pcall(function() if setclipboard then setclipboard(d.message) end end) end}) 
                        end 
                    end 
                end 
            end 
